function banpickD(){
    ...
    let SHEET_TITLE_BAN_D = 'Group D'
    let FULL_URL_D = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?sheet=${SHEET_TITLE_BAN_D}&range=${SHEET_RANGE}`;
    function fetchDataAndUpdate() {
        fetch(FULL_URL_D)
        .then((res) => res.text())
        .then((rep) => {
            let data = JSON.parse(rep.substr(47).slice(0, -2));
            let pickbox = document.querySelectorAll('.pick-box-D');
            let banBoxes = document.querySelectorAll('.ban-box-D'); // Select all elements with the class 'ban-box'
