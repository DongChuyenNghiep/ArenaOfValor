...
let SHEET_RANGE_F_1 = 'A38:J41';
let SHEET_ID = '1yhQbcmnQB52fu1PqlHPRNWOHmJwddS8J9EpIQqvJx2o';
let SHEET_TITLE = 'Fixture-group stage';
let FULL_URL_TABLE_F = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?sheet=${SHEET_TITLE}&range=${SHEET_RANGE_F_1}`;

fetch(FULL_URL_TABLE_F)
.then((res) => res.text())
.then((rep) => {
    let data = JSON.parse(rep.substr(47).slice(0, -2));
    let dataBody = document.getElementById('table_ranktable_F');
...
window.addEventListener('resize', updateTextContentF);
