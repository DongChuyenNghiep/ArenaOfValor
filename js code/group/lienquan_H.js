...
let SHEET_RANGE_H = 'A143:U154';
let SHEET_ID = '1yhQbcmnQB52fu1PqlHPRNWOHmJwddS8J9EpIQqvJx2o';
let SHEET_TITLE_QUALIFIER = 'Fixture';
let FULL_URL_H = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?sheet=${SHEET_TITLE_QUALIFIER}&range=${SHEET_RANGE_H}`;

fetch(FULL_URL_H)
  .then((res) => res.text())
  .then((rep) => {
    let data = JSON.parse(rep.substr(47).slice(0, -2));
    let dataBody = document.getElementById('table_fixture_H');
    ...
    for (let i = 0; i < data.table.rows.length; i++) {
      ...
      tbody_right.id = `team-right-H-${i + 1}`;
